...
class EvoplayCallbackService extends CallbackService
{
    ...
    protected function validateRequest()
    {
        ...
        if ($this->getValue('name') == 'bet' && $this->api->convertCreditsToCurrency($this->user->account->balance) < (float) $this->getValue('data.amount')) {
    ...
    protected function actionBet(): CallbackService
    {
        DB::transaction(function () {
            ...
            if (!$gameable) {
                ...
                AccountTransaction::create($this->user->account, $gameable->game, -$bet, FALSE);
            } else {
                $data = $gameable->data;
            ...
            if ($gameable) {
                ...
                if (!$data->get($callbackId)) {
                    ...
                    if ($win > 0) {
                        ...
                        AccountTransaction::create($this->user->account, $game, $win, FALSE);
                    }
        ...
        DB::transaction(function () {
            ...
            if ($gameable) {
                ...
                if ($data->get($refundCallbackId) && $data->where('refund_callback_id', $refundCallbackId)->count() == 0) {
                    ...
                    AccountTransaction::create($this->user->account, $game, $refundAmount, FALSE);
                }
            }
        });
    ...
    public function getSuccessResponse(): array
    {
        return [
            'status' => 'ok',
            'data' => [
                'balance' => $this->api->convertCreditsToCurrency($this->user->account->balance),
