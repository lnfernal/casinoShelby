<template>
  <v-app :class="navbarVisible ? 'permanent-navbar' : 'temporary-navbar'">
    ...
    <v-navigation-drawer v-if="authenticated" v-model="navigationDrawer" app :permanent="navbarVisible" :temporary="!navbarVisible">
      <v-list-item>
        <v-list-item-content>
      ...
      <v-list dense>
        ...
        <template v-if="authenticated">
          ...
          <v-list-item :to="{ name: 'historySports' }" link exact>
            <v-list-item-action>
              <v-icon>mdi-history</v-icon>
            </v-list-item-action>
            <v-list-item-content>
              <v-list-item-title>Historique sports</v-list-item-title>
            </v-list-item-content>
          </v-list-item>
    ...
    <v-app-bar app :clipped-left="!navbarVisible">
      <v-app-bar-nav-icon v-if="authenticated && !navbarVisible" @click.stop="navigationDrawer = !navigationDrawer" />



      <v-toolbar-title class="d-flex align-center">
        <router-link :to="{ name: 'home' }">
          <v-avatar class="logoShelby:q" width="125" height="40" tile>
            <v-img :src="appLogoUrl" :alt="appName" />
          </v-avatar>
        </router-link>
      </v-toolbar-title>
      <div class="main_header hidden-sm-and-down">
      <v-spacer />

      <!--Nos jeux-->
          <v-toolbar-title class="d-flex align-center">
            <router-link :to="{ name: 'home' }" v-scroll-to="'#section-game'">
              <v-avatar size="24" tile>
                <v-img :src="navLogoGames" :alt="appName" />
              </v-avatar>
          ...
          <!--Wallet-->
          <v-toolbar-title class="d-flex align-center">
            <router-link :to="{ name: 'user.account.deposits.index' }" link exact>
              <v-avatar size="24" tile>
                <v-img :src="navLogoWallet" :alt="appName" />
              </v-avatar>
              <div class="ml-3 d-none d-sm-block">
                <!--{{ $t('Wallet') }}-->
                WALLET
          ...
          <!--Bonus-->
          <v-toolbar-title class="d-flex align-center">
            <router-link :to="{ name: 'home' }" v-scroll-to="'#section-bonus'">
              <v-avatar size="24" tile>
                <v-img :src="navLogoBonus" :alt="appName" />
              </v-avatar>
              <div class="ml-3 d-none d-sm-block">
                <!--{{ $t('Bonus') }}-->
                BONUS
          ...
          <v-toolbar-title class="d-flex align-center">
            <router-link :to="{ name: 'home' }">
              <v-avatar size="24" tile>
                <v-img :src="navLogoApropos" :alt="appName" />
              </v-avatar>
              <div class="ml-3 d-none d-sm-block">
                <!--{{ $t('About') }}-->
                A PROPOS
            ...
            <v-list>
              <v-list-item>
                ...
                <v-list-item-action class="flex-row">
                  ...
                  <v-tooltip v-if="user.kyc == 0" bottom>
                    <template v-slot:activator="{ on }">
                      <v-btn icon to="/api/test" @click="userMenu = false" v-on="on">
                        <v-icon>mdi-link</v-icon>
                      </v-btn>
                    </template>
                    <span>Vérifier mon compte</span>
                  </v-tooltip>
                  <v-tooltip v-if="user.kyc == 1" bottom>
                    <template v-slot:activator="{ on }">
                      <v-btn :disabled="true" icon to="/api/test" @click="userMenu = false" v-on="on">
                        <v-icon>mdi-link</v-icon>
                      </v-btn>
                    </template>
                    <span>Vérification en attente</span>
                  </v-tooltip>
                  <v-tooltip v-if="user.kyc == 2" bottom>
                    <template v-slot:activator="{ on }">
                      <v-btn :disabled="true" icon to="/api/test" @click="userMenu = false" v-on="on">
                        <v-icon>mdi-link</v-icon>
                      </v-btn>
                    </template>
                    <span>Compte vérifier</span>
                  </v-tooltip>
                </v-list-item-action>
              </v-list-item>
            </v-list>
...
export default {
  ...
  computed: {
    ...
    navLogoGames () {
      return config('settings.content.home.header.logo1')
    },
    navLogoWallet () {
      return config('settings.content.home.header.logo2')
    },
    navLogoBonus () {
      return config('settings.content.home.header.logo3')
    },
    navLogoApropos () {
      return config('settings.content.home.header.logo4')
    },
    navbarVisible () {
